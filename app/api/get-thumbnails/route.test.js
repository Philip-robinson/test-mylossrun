...
describe('POST /api/get-thumbnails', () => {
  let POST;

  const sentBody = { pdfId: '9b2f0c52-7c1e-4f7a-9a0d-1c2b3d4e5f6a', page: 1 };
  const fixture = { success: true, thumbnails: ['data:image/png;base64,abc'] };
  ...
  function makeRequest({
    ...
    return new Request('http://localhost/api/get-thumbnails', {
      method: 'POST',
      headers,
  ...
  it('forwards the parsed body and X-Access-Code header to the upstream fetch', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      'https://api.example.com/mylossrun/get-thumbnails',
