...
describe('POST /api/allocate-pdf-id', () => {
  let POST;

  const sentBody = { name: 'loss-run.pdf' };
  const fixture = { pdfId: 'pdf-abc' };
  ...
  function makeRequest({
    ...
    return new Request('http://localhost/api/allocate-pdf-id', {
      method: 'POST',
      headers,
  ...
  // allocate-pdf-id is NOT enveloped upstream; the proxy reads response.arrayBuffer()
  // and passes non-enveloped bodies through unchanged.
  function mockUpstream({ status = 200, data = fixture } = {}) {
    return { status, arrayBuffer: async () => Buffer.from(JSON.stringify(data), 'utf-8') };
  }

  it('forwards { name } and the X-Access-Code header to the upstream fetch', async () => {
    global.fetch.mockResolvedValue(mockUpstream());

    await POST(makeRequest());

    expect(global.fetch).toHaveBeenCalledWith(
      'https://api.example.com/mylossrun/allocate-pdf-id',
  ...
  it('returns the upstream pdfId body and status unchanged', async () => {
    global.fetch.mockResolvedValue(mockUpstream());
